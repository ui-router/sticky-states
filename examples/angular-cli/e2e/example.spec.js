...
test.describe('example app', () => {
  ...
  test('home state has a textarea', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('ui-view[name=home] textarea')).toBeVisible();
  });

  test('retains text entered into the textarea in home', async ({ page }) => {
    await page.goto('/');
    await page.locator('ui-view[name=home] textarea').fill('Text entered here is not lost The quick brown fox');
    ...
    await expect(page.locator('ui-view[name=home] textarea')).toHaveValue(
      'Text entered here is not lost The quick brown fox'
    );
  });
  ...
  test('retains text entered into the textarea in about', async ({ page }) => {
    ...
    await page.locator('ui-view[name=about] textarea').fill('Text entered here is not lost The quack white duck');
    ...
    await expect(page.locator('ui-view[name=about] textarea')).toHaveValue(
      'Text entered here is not lost The quack white duck'
    );
  });
});
